...
enum SHAPE_TYPE {myPOINTS, myLINE_LOOP, myLINE_STRIP, myTRIANGLES};
...
struct myCanvas {
    ...
    flagDrawFilled = false;
    modelingMatrix = glm::mat4(1.f); // identity matrix
    }
    
    void setModelingMatrix(glm::mat4 m) { modelingMatrix = m; }
    
    void enableFilledDraw() { flagDrawFilled = true; }
    void disableFilledDraw() { flagDrawFilled = false; }
    ...
    void draw (int type, vector<mPointf>& vec, mColor vcolor);

    void drawArrays(int type, vector<mPointf>& vert, vector<mColor>& color);

    void drawArrays(int type, float vert[], float color[], int nVert, int nDim);
...
inline float DEG2RAD(float d) { return d*M_PI/180.; }
